...
class CodeReviewAnalyzer:
    ...
    def analyze_code_snippet(
        ...
        try:
            ...
            return {
                ...
                'score': self._calculate_snippet_score(static_analysis, security_issues, performance_issues, quality_issues),
    ...
    def _analyze_performance(self, code: str, language: str) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Nested loops
            if 'for ' in line_stripped and any('for ' in lines[j] for j in range(max(0, i-5), min(len(lines), i+5)) if j != i-1):
...
def create_code_review_analyzer(github_client: GitHubClient, config: Dict[str, Any] = None) -> Optional[CodeReviewAnalyzer]:
    """
