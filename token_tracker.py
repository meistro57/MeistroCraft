...
class TokenUsage:
    ...
    @classmethod
    def from_openai_response(cls, response: Any, model: str, session_id: Optional[str] = None, task_type: Optional[str] = None) -> 'TokenUsage':
    ...
    @classmethod
    def from_anthropic_response(cls, response: Any, model: str, session_id: Optional[str] = None, task_type: Optional[str] = None) -> 'TokenUsage':
    ...
    def _check_limits(self, usage: TokenUsage) -> None:
        ...
        if self.limits.daily_token_limit:
            if daily_summary.total_tokens >= self.limits.daily_token_limit:
                print(f"⚠️  Daily token limit exceeded! Used: {daily_summary.total_tokens}, Limit: {self.limits.daily_token_limit}")
                ...
                print(f"⚠️  Daily token usage at {percentage:.1f}% of limit ({daily_summary.total_tokens}/{self.limits.daily_token_limit})")
        
        # Check cost limits
        if self.limits.daily_cost_limit_usd:
            if daily_summary.total_cost_usd >= self.limits.daily_cost_limit_usd:
                print(f"⚠️  Daily cost limit exceeded! Used: ${daily_summary.total_cost_usd:.4f}, Limit: ${self.limits.daily_cost_limit_usd:.4f}")
                ...
                print(f"⚠️  Daily cost usage at {percentage:.1f}% of limit (${daily_summary.total_cost_usd:.4f}/${self.limits.daily_cost_limit_usd:.4f})")
