...
class MeistroCraftUI:
    ...
    def create_status_panel(self) -> Panel:
        ...
        memory_usage_percent = (self.status.memory_size_mb / self.status.memory_limit_mb) * \
            100 if self.status.memory_limit_mb > 0 else 0
        ...
        status_content.append(
            Text(
                f"Size: {memory_indicator} {self.status.memory_size_mb:.1f}MB / {self.status.memory_limit_mb:.0f}MB",
                style=memory_style))
        ...
        status_content.append(
            Text(
                f"Anthropic: {anthropic_status} {self.status.api_status['anthropic']}",
                style="white"))
    ...
    def run_ui(self, input_callback: Callable[[str], None]) -> None:
        ...
        try:
            with Live(layout, console=self.console, refresh_per_second=10, screen=True) as live:
                ...
                self.add_message(
                    "system", "Welcome to MeistroCraft! Type your request below or use /help for commands.")
    ...
    def _handle_command(self, command: str) -> None:
        ...
        if cmd == '/help':
            ...
            if self.ui.token_tracker:
                ...
                self.ui.add_message(
                    "system", f"Token usage: {summary.total_tokens:,} tokens (${summary.total_cost_usd:.4f})")
            else:
